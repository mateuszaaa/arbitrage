...
std::pair<CrytpoConversionInfo, CrytpoConversionInfo> ConvertToCryptoConversionInfo::convert_to_CrytpoConversionInfo(
PriceInfo info, std::string crypto) {
  ...
  beta2.to = crypto;
  beta2.bid_price = 1 / info.ask_price;
  beta2.bid_quantity = info.ask_quantity;
  return {beta1, beta2};
}

std::vector<std::string> ConvertToCryptoConversionInfo::search_crypto_names(
    std::vector<PriceInfo> &price_infos) {
...
std::vector<CrytpoConversionInfo>
ConvertToCryptoConversionInfo::convert(std::vector<PriceInfo> price_infos) {
